...
app.get("/products/:productName", (req, res) => {
    const prod = products.find(prod => prod.getName() === req.params.productName)
    const prodQuantity = req.session.cart[req.params.productName]
    res.locals = {
        product: prod,
        quantity: prodQuantity ? prodQuantity/prod.getPrice() : 0
    };
    res.render('product');
})
...
app.post("/addToCart", (req, res) => {
    ...
    res.send(200, {quantity: req.session.cart[req.body.name]/prod.getPrice()});
})
